...
def test__init_logging(monkeypatch: MonkeyPatch) -> None:
    # Arrange
    monkeypatch.setattr(
        "sag_py_logging.log_config_initializer._get_config_file_content",
        _get_config_file_content_mock,
    )
    monkeypatch.setattr(
        "sag_py_logging.log_config_initializer._init_python_logging",
        _init_python_logging_mock,
    )
    ...
    actual: dict[str, Any] = init_logging(
        "myconfig.json",
        TestLogLoader(),
        processors=[TestLogProcessorOne(), TestLogProcessorTwo()],
    )
